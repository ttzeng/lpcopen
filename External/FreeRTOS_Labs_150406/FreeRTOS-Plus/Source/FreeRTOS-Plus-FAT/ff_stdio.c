...
uint8_t ucMode;
	...
	{
		/* Now 'xHandler.path' contains an absolute path within the file system.
		Translate a type string "r|w|a[+]" to +FAT's mode bits. */
		ucMode = FF_GetModeBits( pcMode );

		pxStream = FF_Open( xHandler.pxManager, xHandler.path, ucMode, &xError );
...
/*-----------------------------------------------------------*/
	...
	FF_DirHandler_t xHandler;
		...
		else
		{
			...
			iResult = FF_MkDir( xHandler.pxManager, xHandler.path );
	...
	FF_DirHandler_t xHandler;
		...
		else
		{
			...
			{
				/* A simple non-recursive make of a directory. */
				lResult = FF_MkDir( xHandler.pxManager, xHandler.path );
			...
			else
			{
				...
				FF_Error_t errCode;
				int iLength = snprintf( pcTempPath, sizeof pcTempPath, "%s", xHandler.path );
...
FF_DirHandler_t xHandler;
	...
	{
		lResult = FF_RmDir( xHandler.pxManager, xHandler.path );
...
int iReturn, ff_errno;
	...
	{
		xError = FF_RmFile( xHandler.pxManager, xHandler.path );
		ff_errno = prvFFErrorToErrno( xError );
...
int ff_errno = 0, iReturn;
	...
	else
	{
		#if( ffconfigHAS_CWD != 0 )
		{
			xSize = strlen( xHandlers[0].path ) + 1;
			...
			else
			{
				...
				memcpy( pcOldCopy, xHandlers[0].path, xSize );
				xHandlers[0].path = pcOldCopy;
			}
		}
...
#endif /* ffconfigHAS_CWD != 0 */
		{
			...
			{
				xError = FF_Move( xHandlers[ 0 ].pxManager, xHandlers[ 0 ].path, xHandlers[ 1 ].path, bDeleteIfExists );
...
FF_FindParams_t xFindParams;
	...
	if( ( pxStatBuffer != NULL ) && ( FF_isERR( xError ) == pdFALSE ) )
	{
		...
		BaseType_t bIsDeviceDir = xCheckDevicePath( pcFileName );
			...
			{
				FF_Device_GetDirEnt( xHandler.path, &( xDirEntry ) );
			}
		}
		#endif
		...
		pxStatBuffer->st_dev = ( short ) xHandler.fsIndex;
...
const char *pcDirectory;
	...
	if( FF_FS_Find( "ff_findfirst", pcDirectory, &( pxFindData->xDirectoryHandler ) ) == 0 )
	{
	...
	if( iReturn == 0 )
	{
		...
		if( iIsRootDir != pdFALSE )
		{
			...
			such as /ram /nand. */
			pxFindData->xDirectoryHandler.fsIndex = FF_FS_Count();
		...
		pxFindData->xDirectoryHandler.u.bits.bIsValid = 1;
		iReturn = ff_findnext( pxFindData );
	}
	else
	{
		/* errrno has already been set. */
	}
	...
	else
	{
		if( pxFindData->xDirectoryHandler.pxManager != NULL )
		{
			if( pxFindData->xDirectoryHandler.u.bits.bFirstCalled == pdFALSE )
			{
				...
				xError = FF_FindFirst( pxFindData->xDirectoryHandler.pxManager, &( pxFindData->xDirectoryEntry ), pxFindData->xDirectoryHandler.path );
			}
			...
			#if( ffconfigDEV_SUPPORT != 0 )
			{
				...
				{
					FF_Device_GetDirEnt( pxFindData->xDirectoryHandler.path, &( pxFindData->xDirectoryEntry ) );
				}
			}
			#endif
		}
		...
		if( FF_GETERROR( xError ) == FF_ERR_DIR_END_OF_DIR )
		{
			...
			while( ( pxFindData->xDirectoryHandler.fsIndex > 0 ) ||
				   ( pxFindData->xDirectoryHandler.u.bits.bAddDotEntries != 0 ) )
			{
				if( pxFindData->xDirectoryHandler.fsIndex > 0 )
				{
				FF_SubSystem_t xSubSystem;
				int found;

					pxFindData->xDirectoryHandler.fsIndex--;
					found = FF_FS_Get( pxFindData->xDirectoryHandler.fsIndex, &xSubSystem );

					if( ( found == pdFALSE ) || ( xSubSystem.path[ 1 ] == '\0' ) )
					{
						continue;
					}
					snprintf( pxFindData->xDirectoryEntry.pcFileName, sizeof( pxFindData->xDirectoryEntry.pcFileName ), "%s", xSubSystem.path + 1 );
...
int ff_isdirempty(const char *pcPath )
{
	...
	{
		iResult = FF_isDirEmpty( xHandler.pxManager, xHandler.path );
	}
...
FF_Error_t errCode;
	...
	if( FF_FS_Find( "ffFindDir", pcPath, &xHandler ) == pdFALSE )
	{
		/* Return cluster 0 for error. */
		iResult = 0;
	}
	...
	{
		iResult = ( int ) FF_FindDir( xHandler.pxManager, xHandler.path, ( uint16_t ) strlen( xHandler.path ), &errCode );
	}
...
FF_Error_t xReturned;
	...
	else if( xReturned >= SIZE_MAX ) /*_RB_ might generate compiler warnings. */
	{
