...
static xStreamBuffer *xSendBuffer = NULL;
static xStreamBuffer *xRecvBuffer = NULL;
...
static void prvCreateThreadSafeBuffers( void )
{
	...
	if( xSendBuffer == NULL)
	{
		xSendBuffer = ( xStreamBuffer * ) malloc( sizeof( *xSendBuffer ) - sizeof( xSendBuffer->ucArray ) + xSEND_BUFFER_SIZE + 1 );
	...
	if( xRecvBuffer == NULL)
	{
		xRecvBuffer = ( xStreamBuffer * ) malloc( sizeof( *xRecvBuffer ) - sizeof( xRecvBuffer->ucArray ) + xRECV_BUFFER_SIZE + 1 );
...
size_t xSpace;
	...
	xSpace = lStreamBufferGetSpace( xSendBuffer );
		...
		( xSpace >= ( pxNetworkBuffer->xDataLength + sizeof( pxNetworkBuffer->xDataLength ) ) ) )
	{
		...
		itself. */
		lStreamBufferAdd( xSendBuffer, 0, ( const uint8_t * ) &( pxNetworkBuffer->xDataLength ), sizeof( pxNetworkBuffer->xDataLength ) );
		lStreamBufferAdd( xSendBuffer, 0, ( const uint8_t * ) pxNetworkBuffer->pucEthernetBuffer, pxNetworkBuffer->xDataLength );
	}
	else
	{
...
void pcap_callback( u_char *user, const struct pcap_pkthdr *pkt_header, const u_char *pkt_data )
{
	...
	if( ( pkt_header->caplen <= ( ipconfigNETWORK_MTU + ipSIZE_OF_ETH_HEADER ) ) &&
		( lStreamBufferGetSpace( xRecvBuffer ) >= (int32_t)( pkt_header->caplen + sizeof( *pkt_header ) ) ) )
	{
		lStreamBufferAdd( xRecvBuffer, 0, ( const uint8_t* ) pkt_header, sizeof( *pkt_header ) );
		lStreamBufferAdd( xRecvBuffer, 0, ( const uint8_t* ) pkt_data, pkt_header->caplen );
	}
}
/*-----------------------------------------------------------*/
...
const DWORD xMaxMSToWait = 1000;
	...
	for( ;; )
	{
		...
		while( lStreamBufferGetSize( xSendBuffer ) > sizeof( xLength ) )
		{
			lStreamBufferGet( xSendBuffer, 0, ( uint8_t * ) &xLength, sizeof( xLength ), pdFALSE );
			lStreamBufferGet( xSendBuffer, 0, ( uint8_t* ) ucBuffer, xLength, pdFALSE );
...
eFrameProcessingResult_t eResult;
	...
	for( ;; )
	{
		...
		if( lStreamBufferGetSize( xRecvBuffer ) > sizeof( xHeader ) )
		{
			/* Get the next packet. */
			lStreamBufferGet( xRecvBuffer, 0, (uint8_t*)&xHeader, sizeof( xHeader ), pdFALSE );
			lStreamBufferGet( xRecvBuffer, 0, (uint8_t*)ucRecvBuffer, xHeader.len, pdFALSE );
