...
/* A buffer in which file names are created.  Note this buffer is shared by all
the functions in this file, with no mutual exclusion, so the functions in this
file can only be called by a single task. */
static char cFileName[ ffconfigMAX_FILENAME ];
...
int iReturned;
	...
	snprintf( cRAMBuffer, sizeof( cRAMBuffer ), "%s/%s", pcMountPath, "sub1/sub2/sub3/sub4/sub5" );

	/* Attempt to get the CWD, but using a buffer too small.  There is no room
	for the NULL terminator in the line below. */
	configASSERT( ff_getcwd( cFileName, strlen( cRAMBuffer ) ) == NULL );

	/* Ensure the CWD is as expected. */
	configASSERT( ff_getcwd( cFileName, sizeof( cFileName ) ) == cFileName );
	configASSERT( strcmp( cFileName, cRAMBuffer ) == 0 );
...
FF_Stat_t xStat;
	...
	for( x = 0; x < 1000; x++ )
	{
		configASSERT( ff_fgetc( pxFile ) == 0 );
	}
	...
	for( x = 0; x < 1000; x++ )
	{
		configASSERT( ff_fgetc( pxFile ) == 0xff );
	}

	for( x = 0; x < 1000; x++ )
	{
		configASSERT( ff_fgetc( pxFile ) == 0x00 );
	}
...
int iReturned, iExpectedReturn;
	...
	for( xSkippedBytes = 0; xSkippedBytes < xSizeIncrement; xSkippedBytes++ )
	{
		...
		memset( pulVerifyBuffer, 0x00, sizeof( pulVerifyBuffer ) );
...
char cReadBuffer[ 45 ];
	...
	configASSERT( ff_filelength( pxFile ) == 0 );
...
const size_t xBytesToWrite = 10U;
	...
	snprintf( cFileName, ffconfigMAX_FILENAME, "%s/Dummy.txt", pcMountPath );
	pxFile = ff_fopen( cFileName, "r" );
	...
	memset( cRAMBuffer, ( int ) '0', sizeof( cRAMBuffer ) );
	xReturned = ff_fwrite( cRAMBuffer, xBytesToWrite, 1, pxFile );
	...
	/* The file length as reported by ff_stat() should be zer though as the file
	has not yet been committed. */
	ff_stat( cFileName, &xStat );
	...
	pxFile = ff_fopen( cFileName, "a" );
	configASSERT( pxFile );

	/* Write some ascii '1's to the file. */
	memset( cRAMBuffer, ( int ) '1', sizeof( cRAMBuffer ) );
	xReturned = ff_fwrite( cRAMBuffer, 1, xBytesToWrite, pxFile );
	...
	memset( cRAMBuffer, 0x00, sizeof( cRAMBuffer ) );

	/* Read the data into the RAM buffer. */
	ff_fread( cRAMBuffer, ( 2 * xBytesToWrite ), 1, pxFile );
	...
	for( xByte = 0; xByte < ( 2 * xBytesToWrite ); xByte++ )
	{
		...
		{
			configASSERT( cRAMBuffer[ xByte ] == '0' );
		}
		else
		{
			configASSERT( cRAMBuffer[ xByte ] == '1' );
		}
	}

	/* It should not be possible to write to the file as it was opened read
	only. */
	xReturned = ff_fwrite( cRAMBuffer, 1, 1, pxFile );
