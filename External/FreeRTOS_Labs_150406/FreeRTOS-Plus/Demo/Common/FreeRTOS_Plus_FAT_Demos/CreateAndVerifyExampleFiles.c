...
int iReturned;
	...
	for( xFileNumber = 1; xFileNumber <= xMaxFiles; xFileNumber++ )
	{
		...
		{
			lItemsWritten = ff_fwrite( cRAMBuffer, fsRAM_BUFFER_SIZE, 1, pxFile );
...
FF_FILE *pxFile;
	...
	for( xFileNumber = 1; xFileNumber <= xMaxFiles; xFileNumber++ )
	{
		...
		for( xReadNumber = 0; xReadNumber < xFileNumber; xReadNumber++ )
		{
			...
			xItemsRead = ff_fread( cRAMBuffer, fsRAM_BUFFER_SIZE, 1, pxFile );
			...
			{
				configASSERT( cRAMBuffer[ xChar ] == ( '0' + ( char ) xFileNumber ) );
			}
		}
...
FF_FILE *pxFile;
	...
	ff_getcwd( cRAMBuffer, fsRAM_BUFFER_SIZE );
	FF_PRINTF( ( "In directory %s\r\n", cRAMBuffer ) );
	snprintf( cFileName, ffconfigMAX_FILENAME, "%s%s", pcMountPath, pcFullPath );
	configASSERT( strcmp( cRAMBuffer, cFileName ) == 0 );

	/* Generate the file name. */
	snprintf( cFileName, ffconfigMAX_FILENAME, "%s.txt", pcDirectory2 );
	...
	configASSERT( strcmp( cRAMBuffer, pcMountPath ) == 0 );
}
/*-----------------------------------------------------------*/
...
FF_FILE *pxFile;
	...
	/* cFileName is about to be overwritten - take a copy. */
	strcpy( cRAMBuffer, cFileName );
