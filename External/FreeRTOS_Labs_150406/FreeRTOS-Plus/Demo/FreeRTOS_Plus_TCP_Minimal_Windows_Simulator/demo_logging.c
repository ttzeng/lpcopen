...
used). */
static xStreamBuffer *xLogStreamBuffer = NULL;
...
xSocket_t xPrintSocket = FREERTOS_INVALID_SOCKET;
...
HANDLE Win32Thread;
	...
	#if( ( ipconfigHAS_DEBUG_PRINTF == 1 ) || ( ipconfigHAS_PRINTF == 1 ) )
	{
		...
		{
			/* Create the buffer. */
			xLogStreamBuffer = ( xStreamBuffer * ) malloc( sizeof( *xLogStreamBuffer ) - sizeof( xLogStreamBuffer->ucArray ) + dlLOGGING_STREAM_BUFFER_SIZE + 1 );
...
const char *pcNoTask = "None";
	...
	if( ( xStdoutLoggingUsed != pdFALSE ) || ( xDiskFileLoggingUsed != pdFALSE ) )
	{
		...
		xLength2 = lStreamBufferGetSpace( xLogStreamBuffer );
		...
		if( xLength2 >= ( xLength + sizeof( xLength ) ) )
		{
			...
			lStreamBufferAdd( xLogStreamBuffer, 0, ( const uint8_t * ) &( xLength ), sizeof( xLength ) );
			lStreamBufferAdd( xLogStreamBuffer, 0, ( const uint8_t * ) cOutputString, xLength );
...
char cPrintString[ dlMAX_PRINT_STRING_LENGTH ];
	...
	while( lStreamBufferGetSize( xLogStreamBuffer ) > sizeof( xLength ) )
	{
		memset( cPrintString, 0x00, dlMAX_PRINT_STRING_LENGTH );
		lStreamBufferGet( xLogStreamBuffer, 0, ( uint8_t * ) &xLength, sizeof( xLength ), pdFALSE );
		lStreamBufferGet( xLogStreamBuffer, 0, ( uint8_t * ) cPrintString, xLength, pdFALSE );
